"""
reformat_second.py  —  converts a "three-value" log file to results.txt format,
taking the SECOND value from each row.
...
def parse_three_value_file(file_path: str) -> tuple[int | None, dict[str, int]]:
    ...
    for line in lines:
        ...
        # Second value
        second_raw = values[1]
        num = float(re.sub(r'[^\d.]', '', second_raw))
        ms_val = int(num) if second_raw.endswith('ms') else int(num * 1000)

        if name == 'Total':
            total_seconds = int(num) if not second_raw.endswith('ms') else int(num / 1000)
        else:
