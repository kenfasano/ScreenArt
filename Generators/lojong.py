...
class Lojong(Text):
    ...
    def format_text(self, lojong_data: list[dict[str, str]]) -> list[str]:
        ...
        if not filtered_slogans:
            self.log.debug(f"No slogans found for point {point}, picking random sample.")
    ...
    def _load_json_data(self, file_path: str):
        if not os.path.exists(file_path):
            self.log.debug(f"Error: The file '{file_path}' was not found.")
            return None
        ...
        for enc in encodings_to_try:
            try:
                ...
                self.log.debug(f"Unexpected error loading {file_path} with {enc}: {e}")
